...
use purehdf5_format::file_writer::{AttrValue, CompoundTypeBuilder, EnumTypeBuilder, FileWriter};
...
fn h5py_reads_our_array_dataset() {
    ...
    fw.create_dataset("vectors").with_array_data(
        purehdf5_format::type_builders::make_f64_type(), &[3], raw, 2,
    );
    let bytes = fw.finish().unwrap();
...
"#,
    ...
    let addr = purehdf5_format::group_v2::resolve_path_any(&bytes, &sb, "particles").unwrap();
    let hdr = purehdf5_format::object_header::ObjectHeader::parse(&bytes, addr as usize, sb.offset_size, sb.length_size).unwrap();
    let dt_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Datatype).unwrap().data;
    let ds_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Dataspace).unwrap().data;
    let dl_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::DataLayout).unwrap().data;
    let (dt, _) = purehdf5_format::datatype::Datatype::parse(dt_data).unwrap();
    let ds = purehdf5_format::dataspace::Dataspace::parse(ds_data, sb.length_size).unwrap();
    let dl = purehdf5_format::data_layout::DataLayout::parse(dl_data, sb.offset_size, sb.length_size).unwrap();
    let raw = purehdf5_format::data_read::read_raw_data(&bytes, &dl, &ds, &dt).unwrap();
    let fields = purehdf5_format::data_read::read_compound_fields(&raw, &dt).unwrap();
    assert_eq!(fields.len(), 3);
    assert_eq!(fields[0].name, "x");
    let x_vals = purehdf5_format::data_read::read_as_f64(&fields[0].raw_data, &fields[0].datatype).unwrap();
...
"#,
    ...
    let sig = purehdf5_format::signature::find_signature(&bytes).unwrap();
    let sb = purehdf5_format::superblock::Superblock::parse(&bytes, sig).unwrap();
    let addr = purehdf5_format::group_v2::resolve_path_any(&bytes, &sb, "colors").unwrap();
    let hdr = purehdf5_format::object_header::ObjectHeader::parse(&bytes, addr as usize, sb.offset_size, sb.length_size).unwrap();
    let dt_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Datatype).unwrap().data;
    let ds_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Dataspace).unwrap().data;
    let dl_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::DataLayout).unwrap().data;
    ...
    let dl = purehdf5_format::data_layout::DataLayout::parse(dl_data, sb.offset_size, sb.length_size).unwrap();
    let raw = purehdf5_format::data_read::read_raw_data(&bytes, &dl, &ds, &dt).unwrap();
    let names = purehdf5_format::data_read::read_enum_names(&raw, &dt).unwrap();
...
fn read_h5py_generated_ea_file() {
    ...
    let sig = purehdf5_format::signature::find_signature(&bytes).unwrap();
    let sb = purehdf5_format::superblock::Superblock::parse(&bytes, sig).unwrap();
    let addr = purehdf5_format::group_v2::resolve_path_any(&bytes, &sb, "data").unwrap();
    let hdr = purehdf5_format::object_header::ObjectHeader::parse(&bytes, addr as usize, sb.offset_size, sb.length_size).unwrap();
    let dt_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Datatype).unwrap().data;
    let ds_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::Dataspace).unwrap().data;
    let dl_data = &hdr.messages.iter().find(|m| m.msg_type == purehdf5_format::message_type::MessageType::DataLayout).unwrap().data;
    ...
    let dl = purehdf5_format::data_layout::DataLayout::parse(dl_data, sb.offset_size, sb.length_size).unwrap();
    let raw = match &dl {
        purehdf5_format::data_layout::DataLayout::Chunked { .. } => {
            let pipeline = hdr.messages.iter()
                .find(|m| m.msg_type == purehdf5_format::message_type::MessageType::FilterPipeline)
                .map(|m| purehdf5_format::filter_pipeline::FilterPipeline::parse(&m.data).unwrap());
            purehdf5_format::chunked_read::read_chunked_data(&bytes, &dl, &ds, &dt, pipeline.as_ref(), sb.offset_size, sb.length_size).unwrap()
        }
        _ => purehdf5_format::data_read::read_raw_data(&bytes, &dl, &ds, &dt).unwrap(),
    };
    let result = purehdf5_format::data_read::read_as_f64(&raw, &dt).unwrap();
...
fn h5py_reads_dense_attrs() {
    ...
    let bytes = fw.finish().unwrap();
    let path = std::env::temp_dir().join("purehdf5_dense_attrs.h5");
    std::fs::write(&path, &bytes).unwrap();
...
fn h5py_reads_50_dense_attrs() {
    ...
    let bytes = fw.finish().unwrap();
    let path = std::env::temp_dir().join("purehdf5_dense_50_attrs.h5");
    std::fs::write(&path, &bytes).unwrap();
