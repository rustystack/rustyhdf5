...
use purehdf5_format::fractal_heap::FractalHeapHeader;
use purehdf5_format::btree_v2::BTreeV2Header;
...
fn corrupted_file_detected() {
    ...
    let mut fw = purehdf5_format::file_writer::FileWriter::new();
...
fn extract_attrs_from_dataset_no_panic() {
    ...
    let attrs = purehdf5_format::attribute::extract_attributes_full(
...
fn provenance_mismatch_on_corruption() {
    ...
    let sig = signature::find_signature(&bytes).unwrap();
    let sb = Superblock::parse(&bytes, sig).unwrap();
    let addr = purehdf5_format::group_v2::resolve_path_any(&bytes, &sb, "sensor").unwrap();
    let hdr = ObjectHeader::parse(&bytes, addr as usize, sb.offset_size, sb.length_size).unwrap();
    let result = purehdf5_format::provenance::verify_dataset(
        &bytes, &hdr, sb.offset_size, sb.length_size,
    ).unwrap();
    assert!(
        matches!(result, purehdf5_format::provenance::VerifyResult::Mismatch { .. }),
