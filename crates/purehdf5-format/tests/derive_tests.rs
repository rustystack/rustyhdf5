...
use purehdf5_derive::H5Type;
use purehdf5_format::datatype::Datatype;
use purehdf5_format::data_read::{read_as_f64, read_as_i32, read_compound_fields};
...
fn roundtrip_through_file_writer() {
    ...
    use purehdf5_format::data_read::read_raw_data;
    ...
    for msg in &oh.messages {
        if msg.msg_type == purehdf5_format::message_type::MessageType::Link {
            let link = purehdf5_format::link_message::LinkMessage::parse(&msg.data, sb.offset_size).unwrap();
            if link.name == "compound_ds" {
                if let purehdf5_format::link_message::LinkTarget::Hard { object_header_address } = link.link_target {
    ...
    for msg in &ds_oh.messages {
        match msg.msg_type {
            purehdf5_format::message_type::MessageType::Datatype => {
                let (parsed_dt, _) = purehdf5_format::datatype::Datatype::parse(&msg.data).unwrap();
                found_dt = Some(parsed_dt);
            }
            purehdf5_format::message_type::MessageType::Dataspace => {
                found_ds = Some(purehdf5_format::dataspace::Dataspace::parse(&msg.data, sb.length_size).unwrap());
            }
            purehdf5_format::message_type::MessageType::DataLayout => {
                found_layout = Some(purehdf5_format::data_layout::DataLayout::parse(&msg.data, sb.offset_size, sb.length_size).unwrap());
            }
            _ => {}
        }
    }
