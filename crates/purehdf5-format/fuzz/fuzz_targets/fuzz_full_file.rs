...
fuzz_target!(|data: &[u8]| {
    ...
    let sig = match purehdf5_format::signature::find_signature(data) {
        Ok(s) => s,
        Err(_) => return,
    };
    let sb = match purehdf5_format::superblock::Superblock::parse(data, sig) {
        Ok(s) => s,
        Err(_) => return,
    };
    ...
    let _ = purehdf5_format::group_v2::resolve_path_any(data, &sb, "/");
});
