...
//!   cargo bench -p purehdf5-filters --features lz4,zstd --bench compression_bench
...
fn bench_lz4(c: &mut Criterion) {
    ...
    c.bench_function("lz4_compress_8MB", |b| {
        b.iter(|| purehdf5_filters::lz4_compress(black_box(&data)).unwrap())
    });

    let compressed = purehdf5_filters::lz4_compress(&data).unwrap();
    c.bench_function("lz4_decompress_8MB", |b| {
        b.iter(|| purehdf5_filters::lz4_decompress(black_box(&compressed)).unwrap())
    });
}
...
fn bench_zstd(c: &mut Criterion) {
    ...
    c.bench_function("zstd_compress_level1_8MB", |b| {
        b.iter(|| purehdf5_filters::zstd_compress(black_box(&data), 1).unwrap())
    });

    c.bench_function("zstd_compress_level3_8MB", |b| {
        b.iter(|| purehdf5_filters::zstd_compress(black_box(&data), 3).unwrap())
    });

    let compressed = purehdf5_filters::zstd_compress(&data, 3).unwrap();
    c.bench_function("zstd_decompress_8MB", |b| {
        b.iter(|| purehdf5_filters::zstd_decompress(black_box(&compressed)).unwrap())
    });
}
...
fn bench_parallel_deflate(c: &mut Criterion) {
    ...
    c.bench_function("deflate_sequential_10chunks", |b| {
        b.iter(|| {
            ...
            purehdf5_filters::compress_chunks(black_box(&refs), |data| {
                purehdf5_filters::deflate_compress(data, 6)
            })
            .unwrap()
        })
    });
