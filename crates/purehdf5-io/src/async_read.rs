...
use purehdf5_format::object_header::{HeaderMessage, ObjectHeader};
...
/// the synchronous purehdf5-format parsers (which operate on `&[u8]`).
#[derive(Debug)]
...
mod tests {
    ...
    async fn async_file_reader_roundtrip() {
        ...
        let path = dir.join("purehdf5_async_file_reader_test.bin");
    ...
    async fn async_file_reader_open_loaded() {
        let dir = std::env::temp_dir();
        let path = dir.join("purehdf5_async_file_reader_loaded_test.bin");
    ...
    async fn async_file_reader_nonexistent() {
        let reader = AsyncFileReader::new("/tmp/purehdf5_async_does_not_exist_12345.bin");
    ...
    async fn async_hdf5_file_open_path() {
        let dir = std::env::temp_dir();
        let path = dir.join("purehdf5_async_hdf5_open_path.h5");
    ...
    async fn async_loaded_reader_into_inner() {
        let dir = std::env::temp_dir();
        let path = dir.join("purehdf5_async_loaded_into_inner.bin");
