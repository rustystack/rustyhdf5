"""Tests for purehdf5 Python bindings."""
...
def sample_read_file(tmp_h5):
    ...
    with purehdf5.File(tmp_h5, "w") as f:
...
def test_open_and_read_f64(sample_read_file):
    f = purehdf5.File(sample_read_file, "r")
    ds = f["temperatures"]
    data = ds[:]
...
def test_dataset_shape(sample_read_file):
    with purehdf5.File(sample_read_file, "r") as f:
        ds = f["temperatures"]
        assert ds.shape == (3,)
...
def test_read_group_keys(grouped_read_file):
    with purehdf5.File(grouped_read_file, "r") as f:
        keys = f.keys()
...
def test_write_simple(tmp_h5):
    ...
    with purehdf5.File(tmp_h5, "r") as f:
        data = f["data"][:]
...
def test_open_nonexistent_file():
    with pytest.raises(OSError):
        purehdf5.File("/nonexistent/path.h5", "r")


def test_invalid_mode(tmp_h5):
    with pytest.raises(ValueError):
        purehdf5.File(tmp_h5, "x")
