...
enum AttrsInner {
    ...
    Read(HashMap<String, purehdf5_rs::AttrValue>),
...
impl PyAttrs {
    ...
    pub(crate) fn from_read(map: HashMap<String, purehdf5_rs::AttrValue>) -> Self {
        Self {
        ...
        match &self.inner {
            ...
            AttrsInner::Write(store) => {
                ...
                for (k, v) in guard.iter() {
                    if k == key {
                        let attr_val: purehdf5_rs::AttrValue = v.clone().into();
    ...
    fn values(&self, py: Python<'_>) -> PyResult<PyObject> {
        let vals: Vec<PyObject> = match &self.inner {
            ...
            AttrsInner::Write(store) => store
                ...
                .iter()
                .map(|(_, v)| {
                    let attr: purehdf5_rs::AttrValue = v.clone().into();
...
mod tests {
    ...
    fn read_attrs_len() {
        ...
        map.insert("a".into(), purehdf5_rs::AttrValue::I64(1));
        map.insert("b".into(), purehdf5_rs::AttrValue::F64(2.0));
    ...
    fn read_attrs_contains() {
        let mut map = HashMap::new();
        map.insert("x".into(), purehdf5_rs::AttrValue::String("hello".into()));
        let attrs = PyAttrs::from_read(map);
