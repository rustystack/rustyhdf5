...
/// ```python
/// # Reading
/// f = purehdf5.File('data.h5', 'r')
/// ds = f['dataset']
/// f.close()
///
/// # Writing
/// with purehdf5.File('out.h5', 'w') as f:
...
impl PyFile {
    ...
    fn new(path: &str, mode: &str) -> PyResult<Self> {
        match mode {
            "r" => {
                let file = purehdf5_rs::File::open(path).map_err(to_py_err)?;
                Ok(Self {
...
impl PyFile {
    fn read_file(&self) -> PyResult<&Arc<purehdf5_rs::File>> {
        match &self.inner {
...
mod tests {
    ...
    fn finalize_roundtrip() {
        ...
        let path = dir.join("purehdf5_py_test_finalize.h5");
        ...
        let file = purehdf5_rs::File::open(&path).unwrap();
