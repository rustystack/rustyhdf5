...
pub use purehdf5_format::file_writer::AttrValue;

/// Simplified datatype enum for the high-level API.
///
/// Maps from the detailed `purehdf5_format::datatype::Datatype` to a
...
pub(crate) fn classify_datatype(dt: &purehdf5_format::datatype::Datatype) -> DType {
...
pub(crate) fn attrs_to_map(
    attrs: &[purehdf5_format::attribute::AttributeMessage],
    file_data: &[u8],
    offset_size: u8,
    length_size: u8,
...
fn decode_attr_value(
    attr: &purehdf5_format::attribute::AttributeMessage,
    file_data: &[u8],
    offset_size: u8,
    length_size: u8,
) -> Option<AttrValue> {
