//! NetCDF-4 read support built on purehdf5.
//!
...
//!
//! ```no_run
//! use purehdf5_netcdf4::NetCDF4File;
//!
...
///
/// Wraps a purehdf5 File and provides NetCDF-4 semantics: dimensions,
...
impl NetCDF4File {
    ...
    pub fn open<P: AsRef<std::path::Path>>(path: P) -> Result<Self, Error> {
        let hdf5 = purehdf5::File::open(path)?;
        Ok(Self { hdf5 })
    }

    /// Open a NetCDF-4 file from in-memory bytes.
    pub fn from_bytes(data: Vec<u8>) -> Result<Self, Error> {
        let hdf5 = purehdf5::File::from_bytes(data)?;
        Ok(Self { hdf5 })
    }
    ...
    pub fn hdf5_file(&self) -> &purehdf5::File {
        &self.hdf5
    }
}
