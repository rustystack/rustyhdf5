//! Comprehensive tests for purehdf5-netcdf4.
//!
//! We create NetCDF-4 style HDF5 files using purehdf5's FileBuilder with the
//! appropriate NetCDF-4 conventions (CLASS=DIMENSION_SCALE, _Netcdf4Dimid,
//! _NCProperties, CF attributes, etc.).

use purehdf5::{AttrValue, FileBuilder};
use purehdf5_netcdf4::{Dimension, FillValue, NcType, NetCDF4File};
...
fn make_simple_netcdf4() -> Vec<u8> {
    ...
    b.set_attr(
        "_NCProperties",
        AttrValue::String("version=2,purehdf5=0.1.0".into()),
    );
    b.set_attr(
        "Conventions",
    ...
    b.set_attr(
        "history",
        AttrValue::String("created by purehdf5-netcdf4 test".into()),
    );
...
fn test_nc_properties() {
    ...
    assert!(props.unwrap().contains("purehdf5"));
}
...
fn test_global_attrs() {
    ...
    assert!(matches!(
        attrs.get("history"),
        Some(AttrValue::String(s)) if s.contains("purehdf5-netcdf4")
    ));
}
...
fn test_open_from_disk() {
    let bytes = make_simple_netcdf4();
    let path = std::env::temp_dir().join("purehdf5_netcdf4_test.nc");
